...
$(function () {
    $(document).on("click", '[data-inline-delete="true"]', function () {
        ...
        var isDelete = false;
        if (trim(clickedTagData.isDelete) != "") {
            isDelete = clickedTagData.isDelete;
        }

        var isReload = false;
        if (trim(clickedTagData.isReload) != "") {
            isReload = clickedTagData.isReload;
        }
            ...
            function () {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        // on the case of delete remove the row 
                        if (isDelete) {
...
$(function () {
    $("form[data-post-type='ajax'][ method=\"post\"]").find(':submit').on("click", function () {
        ...
        var validationFaliurMethod = form.data("validation-faluire-method");
        ...
        if (isValid) {
            ...
            if (trim(validationFaliurMethod) != "") {
                window[validationFaliurMethod](form);
            }
        }
...
function checkUrlForInnerpages(url, nav) {
    ...
    if (url.indexOf("/dashboard/InvitedTestCandidateTestResults") != -1) {
...
function WholeNumberConvertor() {
    $('wnumber').each(function () {
        var wholenumber = parseInt($(this).text());
        $(this).text(wholenumber);
    });
}

// check  on key up that email is valid or not in send invitation page 
$('#EmailFromEmailAddress').on('focusout keyup', function () {
    if ($('#submitTestQuestionDetails').valid()) {
        // do something
        $("#alert-email-required").addClass("d-none");
    }
    else {
        $("#alert-email-required").removeClass("d-none");
    }
    $("#alert-fail-invitation").addClass("d-none");
});
// for invite test form validation 
function ValidationFailSendInvitation() {
    $("#alert-email-required").removeClass("d-none");
}
//


function AjaxOnSendIvitationPageWithSuccessMessage(data, form) {
    var frmData = $(form).data();
    var succeessMessageDiv = frmData.successSelector;
    var failMessageDiv = frmData.failSelector;
    $("#" + succeessMessageDiv).addClass('d-none');
...
//############################# for top menu toggle- in show or hide stage  ####################
$(document).ready(function () {
    if ($("#closeMenu").length > 0) {
        $("#menuToggleButton").removeClass("collapsed");
